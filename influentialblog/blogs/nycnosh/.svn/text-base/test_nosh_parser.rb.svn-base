...
class Nycnosh 
  ...
  def parse(in_url,log)
    ...
    #get title
    t = doc.at("//div.post/h3.storytitle/a")
    if t.nil?
    ...
    #get date
    dhead = doc.at("//div#content/h2")
    if dhead.nil?
      puts "can't find date"
    else
      dh = dhead.inner_text.strip
    end
    
    d = doc.at("//div.post/div.meta")
    if d.nil?
      ...
      if dd =~ /([^@]+)@(.*)/
        dt = $2.strip
          ...
          puts "Can't parse time"
      end
    end
    if dh.nil? or dh.empty?
      puts "no date??"
    else
      date = Time.parse(dh+" "+dt).strftime("%Y-%m-%d %H:%M:%S")
    end
    ...
    doc.search("//ol[@id='commentlist']/li").each do|each_c|
      ...
      (each_c/"/p/cite").remove
      
      cc = each_c.inner_text
      if cc.nil?
...
def test_main
  test_url = "http://www.nycnosh.com/?p=91"
