...
class Writingwithmouthfull
  ...
  def parse(in_url,log)
    ...
    #get title
    t = doc.at("//div#pagina/h1.post-titulo")
    if t.nil?
    ...
    #get date: no post date in its web page
    if in_url =~ /http:\/\/www.writingwithmymouthfull.com\/(\d+)\/(\d+)\/(\d+)\/.*/
      d = $1+"-"+$2+"-"+$3 
      date = Time.parse(d).strftime("%Y-%m-%d %H:%M:%S")
    else
      puts "Can't parse pots date"
    end
    ...
    #get content
    content = ""
    doc.search("//div.postentry//style").remove
    cdiv = doc.at("//div.postentry")
    if cdiv.nil?
    ...
    doc.search("//div.comentarios").each do|each_c|
      commentHash=Hash.new
      ca = each_c.at("/div/h3")
      if ca.nil?
        puts "Can't find comment's author"
      else
        if ca.inner_text.strip =~ /(.*)\s+said,/
      ...
      cd = each_c.at("/div/p.comentariosmeta")
      if cd.nil?
      ...
      (each_c/"/div/p.comentariosmeta").remove
      each_c.search("/div/p").each do |para|
        if para.nil?
...
def test_main
  test_url = "http://www.writingwithmymouthfull.com/2009/05/27/pylos-greek-restaurant-review/"
...
def main
  log_file=File.open("mouthfull.log","a+")
...
blogparser_pipeline(Writingwithmouthfull)
