...
class Cityeating
  ...
  def parse(in_url,log)
    ...
    #get title
    t = doc.at("//div.entry/h3.entry-header")
    if t.nil?
    ...
    #get date
    d_head = doc.at("//div.pkg/h2.date-header")
    if d_head.nil?
      puts "Can't find date!"
    else
      dh = d_head.inner_text.strip 
      d = doc.at("//div.entry/div.entry-footer/p.entry-footer-info/span.post-footers")
      if d.nil?
        puts "Can't find date!"
      else
        dd = d.inner_text.strip.gsub(/\s+/," ")
        if dd =~ /Posted at(.*)in.*/
          d_date = $1.strip
          date = Time.parse(dh+" "+d_date).strftime("%Y-%m-%d %H:%M:%S")
        else
   ...
   doc.search("//div#comments-content/div[@class*='comment']").each do |each_c|
    commentHash=Hash.new
    
    ca = (each_c/"/p.comment-footer")
    if ca.nil?
      puts "Can't  find comment author and date"
    else
      ca_text =  ca.inner_text.strip.gsub(/\s+/," ")
      if ca_text =~ /Posted by:(.*)\|\s+(.*)/
        commentHash['author']=$1.strip
        c_date = $2.strip
        commentHash['author']=commentHash['author'].gsub(/\\/,"").gsub("\"","\\\"")
        commentHash['date']=Time.parse(c_date).strftime("%Y-%m-%d %H:%M:%S")
      else
    ...
    if cc.nil?
      ...
      commentHash['content'] = cc.inner_text.strip.gsub(/\\/,"").gsub("\"","\\\"")
    end
...
def test_main
  test_url = "http://blog.city-eating.com/2008/11/world-burger-week-at-eagle-bar-diner.html"
...
def main
  log_file=File.open("cityeating.log","a+")
...
blogparser_pipeline(Cityeating)
