  ...
  doc.search("//div[@id='comments']/ol/li").each do |comment_entry|
    ...
    comment_date = comment_entry.at("//div[@class='comment-meta']/span[@class='updated']")
      ...
      if author.nil?
        ...
        comm_hash[:author] = author.inner_text.gsub("—","").gsub(";","").gsub("\\","").strip
      end
...
def parse(file_name)
  ...
  post_content = ""
  if !content.nil?
    post_content = content.inner_text.gsub('"','\"').strip
  end
  ...
  link_hash["#{url}"] = links
  $link_array << link_hash

  post_hash[:date] = blog_date
  post_hash[:author] = author
  post_hash[:url] = url
  post_hash[:title] = title
  post_hash[:post] = post_content
  $post_array << post_hash
  
  #get comments
  comments = Array.new
  comments += getcomments(doc)
  prefix = "http://dinersjournal.blogs.nytimes.com/"
  ...
  log=open("/ursa/local/jinyun/dinerjournal/pagelog", "a+")
  log.puts("URL:")
  log.puts(url)
  log.puts("Title:")
  log.puts(title)
  ...
  if blog_exist.empty?
    sql = <<-SQL
            ...
            values ("4",null,"#{url}","#{blog_date}","#{author}","#{post_content}","#{title}")
    SQL
