...
def parse(in_url, log)
  ...
  if ddate.empty? or ddate.nil?
    ...
    date = Time.parse(ddate+dtime).strftime("%Y-%m-%d %H:%M:%S")
  end
  #get content
...
def test_main
  test_url ="http://www.roboppy.net/food/2008/10/kare-week-day-2-lunch-momofuku-ssam-bar-staten-island-ferry.html"
