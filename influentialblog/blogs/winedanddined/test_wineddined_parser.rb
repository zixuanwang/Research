...
class Winedanddined
  ...
  def parse(in_url,log)
    ...
    #get date
    dd = doc.at("//div.post/p.date")
    if dd.nil?
      puts "Can't find date!"
    else
      d_date = dd.inner_text.strip.gsub(/\s+/, " ")
    end
    da = doc.at("//div.post/div.meta/p")
    if da.nil?
      puts "can't find time and author"
    else
      if da.inner_text.strip =~ /Published by(.*)at(.*)under(.*)/
        author=$1.strip
        d_time=$2.strip
        date = Time.parse(d_date+" "+d_time).strftime("%Y-%m-%d %H:%M:%S")
     else
    ...
    #get content
    doc.search("//div.post/div.entry/p/script").remove
    doc.search("//div.post/div.entry/p/span#sharethis_0").remove
    content = ""
    cdiv = doc.at("//div.post/div.entry")
    if cdiv.nil?
    ...
    doc.search("//ol.commentlist/li").each do|each_c|
      commentHash=Hash.new
      ca = each_c.at("/div[@class='cmtinfo']/cite")
      if ca.nil?
      ...
      cd = each_c.at("/div[@class='cmtinfo']/em")
      if cd.nil?
        puts "Can't find comment's author and date"
      else
      ...
      each_c.search("/div[@class='cmtinfo']").remove
      cc = each_c.at("/p")
      if cc.nil?
...
def test_main
  test_url = "http://www.winedanddined.com/2009/01/restaurant-week-2009-ny/"
...
def main
  log_file=File.open("winedine.log","a+")
...
blogparser_pipeline(Winedanddined)
