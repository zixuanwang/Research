...
class Downbythehipster 
  ...
  def parse(in_url,log)
    ...
    #get title
    t = doc.at("//div.journal-entry-text/h2.title")
    if t.nil?
    ...
    #get date and author
    a = doc.at("//div[@class='journal-entry-tag journal-entry-tag-post-body']/div.journal-entry-tag-post-body-line1/span.posted-by")
    if a.nil?
    ...
    d= doc.at("//div[@class='journal-entry-tag journal-entry-tag-post-body']/div.journal-entry-tag-post-body-line1/span.posted-on")
    if d.nil?
    ...
    #get content
    content = ""
    cdiv = doc.at("//div.journal-entry-text/div.body")
    if cdiv.nil?
   ...
   doc.search("//div.journal-comment-area/div/div.comment").each do |each_c|
    commentHash = Hash.new
    ca = (each_c/"//div.signature")
    if ca.nil?
      puts "Can't  find comment author "
    else
      ca_text = ca.inner_text.strip.gsub(/\s+/," ")
      if ca_text =~ /(.*)\|\s+(.*)/
        commentHash['author']=$2.strip
        cd = $1.strip
        commentHash['author']=commentHash['author'].gsub(/\\/,"").gsub("\"","\\\"")
        commentHash['date']=Time.parse(cd).strftime("%Y-%m-%d %H:%M:%S")
      else
        puts "Can't parse comment author and date: #{ca_text}"
      end
    end
    cc =(each_c/"/div.body")
    if cc.nil?
...
def test_main
  test_url = "http://www.downbythehipster.com/blog/2009/5/29/harbor-club-packs-its-bags-for-georgica.html"
...
blogparser_pipeline(Downbythehipster)
