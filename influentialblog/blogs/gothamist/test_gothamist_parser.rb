...
class Gothamist 
  ...
  def parse(in_url,log)
    ...
    #get title
    t = doc.at("//div.asset-header/h1#page-title")
    if t.nil?
    ...
    #get date and author
    dd = doc.at("//div.asset-footer/div.asset-meta/span.byline")
    if dd.nil?
      puts "Can't find date and author!"
    else
      ddd = dd.inner_text.strip.gsub(/\s+/," ")
      if ddd =~ /^By(.*)\s+in(.*)\s+on(.*)/
        d_date = $3.strip
        author = $1.strip
   ...
   doc.search("//div.comments-content/div/div.inner").each do |each_c|
    commentHash=Hash.new
    ca = (each_c/"//div.asset-meta/span.byline/span[@class='vcard author']")
    if ca.nil?
      ...
      if ca_text =~ /(.*)\|\s+(.*)/
        ...
        commentHash['author']=ca_text
        commentHash['author']=commentHash['author'].gsub(/\\/,"").gsub("\"","\\\"")
      end
    end
    cd = each_c.at("/div.comment-footer/div.asset-meta/a.comment_date/abbr.published")
    if cd.nil?
      cdd = each_c.at("/div.comment-footer/div.asset-meta/a/abbr.published")
      if cdd.nil?
        puts "Can't find comment date"
      else
        commentHash['date']=Time.parse(cdd.inner_text.strip).strftime("%Y-%m-%d %H:%M:%S")
      end
    else
...
def test_main
  test_url = "http://gothamist.com/2003/02/20/no_hojo_dont_go.php"
...
def main
  log_file=File.open("gothamist.log","a+")
