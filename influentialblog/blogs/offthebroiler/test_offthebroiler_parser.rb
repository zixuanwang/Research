...
class Offthebroiler 
  ...
  def parse(in_url,log)
    ...
    #get date
    d = doc.at("//div.entry/p[@class='postmetadata alt']/small")
    if d.nil?
      puts "Can't find date!"
    else
      if d.inner_text.strip =~ /This entry was posted  on(.*)and(.*)/
        date = Time.parse($1.strip).strftime("%Y-%m-%d %H:%M:%S")
      else
    ...
    #get content
    content = ""
    doc.search("//div.postentry/").remove
    cdiv = doc.at("//div.snap_preview")
    if cdiv.nil?
    ...
    doc.search("//div#content/ol[@class='commentlist snap_preview']/li").each do|each_c|
      if each_c.inner_html.empty? or each_c.inner_html.to_s == "" 
        next
      end
      ...
      cd = each_c/"/small.commentmetadata"
      if cd.nil?
        puts "Can't find comment date"
      else
        if cd.inner_text.strip =~ /([^|]+)\| Reply(.*)/
          commentHash['date']=Time.parse($1.strip).strftime("%Y-%m-%d %H:%M:%S")
        else
      ...
      if para.nil?
          ...
          begin
            ...
            puts "drop comment content #{encoding}"
            badurls.puts("EMPTY COMMENT: #{encoding} #{in_url}")
            badurls.close
          rescue ArgumentError=>arg
...
def test_main
  test_url = "http://offthebroiler.wordpress.com/2007/06/26/im-with-mario/"
...
def main
  log_file=File.open("offthebroiler.log","a+")
...
blogparser_pipeline(Offthebroiler)
