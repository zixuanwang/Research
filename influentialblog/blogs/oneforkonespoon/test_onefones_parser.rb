...
def parse(in_url, log)
  ...
  if loopnumber>0
    ...
    i=0
    doc.search("//dl[@id='comments-block']/dt[@class='comment-author blogger-comment-icon']/a[@rel='nofollow']").each do|a|
      commentarray[i]['author']=a.inner_text.strip
      i+=1
    end
...
def test_main
  test_url ="http://oneforkonespoon.blogspot.com/2007/11/meat-and-potatoes-meat-and-potatoes.html"
...
def main
  log_file=File.open("oneforkonespoon.log","a+")
