...
class Thefoodsection
  ...
  def parse(in_url,log)
    ...
    #get content
    doc.search("//div.item/div.links").remove
    doc.search("//div.item/div.cl").remove
    doc.search("//div.item/h1").remove
    content = ""
    cdiv = doc.at("//div.item")
    if cdiv.nil?
...
def test_main
  test_url = "http://www.thefoodsection.com/foodsection/2009/05/live-from-the-james-beard-awards.html"
...
def main
  log_file=File.open("foodsection.log","a+")
...
blogparser_pipeline(Thefoodsection)
