...
def parse(in_url, log)
  ...
  #get date-time
  dd = doc.at("//div[@class='post-footer']/p/span[@class='post-timestamp']/a/abbr")
  ...
  if loopnumber>0
    ...
    doc.search("//dl[@id='comments-block']/dd[@class='comment-body']").each do|c|
      ...
      commentarray[i]['content']=commentarray[i]['content'].gsub(/\\/,"").gsub("\"","\\\"").gsub(/\s+/," ")
      i=i+1
    end
...
def test_main
  test_url = "http://nyctastes.blogspot.com/2009/05/yankee-eats.html"
...
def main
  log_file=File.open("nyctastes.log","a+")
