...
def parse(in_url, log)
  ...
  #get date_header 
  dh = doc.at("//div[@class='blog-posts hfeed']/h2[@class='date-header']")
  if dh.nil?
    puts "Can't find date header"
  else
    date_header = dh.inner_text.strip
  end
  #get comments
  ...
  if loopnumber>0
    ...
    doc.search("//dl[@id='comments-block']/dd[@class='comment-footer']/span[@class='comment-timestamp']/a").each do |t|
      cdd = date_header +" "+ t.inner_text.strip
...
def test_main
  test_url="http://apassionforfood.blogspot.com/2008/03/cake-batchpizza-artichokeeternal.html"
...
def main
  log_file=File.open("passion.log","a+")
...
blogparser_pipeline(Apassionforfood)
