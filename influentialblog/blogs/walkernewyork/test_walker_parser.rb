...
class Walkernewyork
  ...
  def parse(in_url,log)
    ...
    #get date
    dd = doc.at("//div.asset-meta/span.byline")
    if dd.nil?
      puts "Can't find date!"
    else
      d_date = dd.inner_text.strip
   ...
   doc.search("//div.comments/div.comments-content/div.comment").each do |each_c|
    commentHash=Hash.new
    ca = (each_c/"/div.inner/div.comment-header")
    if ca.nil?
    ...
    cd = (each_c/"/div.inner/div.comment-footer")
    if cd.nil?
    ...
    cc =(each_c/"/div.inner/div.comment-content")
    if cc.nil?
...
def test_main
  test_url = "http://www.walkernewyork.com/eats/2004/03/all-i-wanted.html"
...
def main
  log_file=File.open("walker.log","a+")
...
blogparser_pipeline(Walkernewyork)
