...
public:
	...
	cv::Mat getLandmark(cv::vector<cv::Point2f>* pLandmarkArray,
	...
	cv::Mat normalize(std::vector<cv::Point2f>* pLandmarkArray,
			const cv::Mat& image, const cv::Rect& bbox,
	...
	cv::Mat warp(std::vector<cv::Point2f>* pLandmarkArray, const cv::Mat& image,
			const std::vector<cv::Point2f>& landmarkArray);
	// show landmarks for debugging.
	void draw(cv::Mat *pImage, const std::vector<cv::Point2f>& landmarkArray,
