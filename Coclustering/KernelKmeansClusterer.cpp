...
KernelKmeansClusterer::KernelKmeansClusterer(int k, int maxIteration): Clusterer(k), mMaxIteration(maxIteration)
{
}
...
float KernelKmeansClusterer::distanceToCluster(int sampleId, const boost::unordered_set<int>& clusterIdSet, const boost::shared_ptr<Kernel>& pKernel){
	...
	for(boost::unordered_set<int>::const_iterator iter=clusterIdSet.begin();iter!=clusterIdSet.end();++iter){
		float weight=mWeightArray[*iter];
		weightSum+=weight;
		float kernelResult=(*pKernel)(sampleId,*iter);
		kernelSum+=weight*kernelResult;
		for(boost::unordered_set<int>::const_iterator _iter=clusterIdSet.begin();_iter!=clusterIdSet.end();++_iter){
	...
	return kernelSelf-2*kernelSum/weightSum+kernelPairSum/(weightSum*weightSum);
}
...
void KernelKmeansClusterer::cluster(){
	...
	for(int iter=0;iter<mMaxIteration;++iter){
		...
		for(int i=0;i<sampleCount;++i){
			...
			rankList.reserve(mK);
			for(int j=0;j<mK;++j){
				float distance=distanceToCluster(i,clusterIdSetMap[j],pKernel);
				RankItem<int, float> item(j,distance);
				rankList.push_back(item);
			}
			int oldLabel=mLabelArray[i];
			int newLabel=std::min_element(rankList.begin(),rankList.end())->index;
