...
void Vocabulary::build(const cv::Mat& sampleMat, int clusterCount) {
	...
	cv::kmeans(sampleMat, clusterCount, clusters,
			cv::TermCriteria(CV_TERMCRIT_EPS + CV_TERMCRIT_ITER, 50, 1e-6), 5,
			cv::KMEANS_PP_CENTERS, mMat);
}
...
void Vocabulary::quantize(std::vector<float>* pVector,
		const cv::Mat& descriptor) {
	cv::Mat indexMat;
	cv::Mat distanceMat;
	mpIndex->knnSearch(descriptor, indexMat, distanceMat, 1,
