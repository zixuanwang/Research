...
FLANDMARK_Model * flandmark_init(const char* filename) {
	...
	fscan_ret = fscanf(fin, " %d %d ", &tst->data.options.bw[0],
			&tst->data.options.bw[1]);
	fscan_ret = fscanf(fin, " %d %d ", &tst->data.options.bw_margin[0],
	...
	fscan_ret = fscanf(fin, " %d %d ", &tst->data.imSize[0],
			&tst->data.imSize[1]);

	int M = tst->data.options.M;

	tst->data.lbp = (FLANDMARK_LBP*) malloc(
			tst->data.options.M * sizeof(FLANDMARK_LBP));
	for (int idx = 0; idx < tst->data.options.M; ++idx) {
		fscan_ret = fscanf(fin, " %d %d ", &tst->data.lbp[idx].WINS_ROWS,
				&tst->data.lbp[idx].WINS_COLS);
	}
	for (int idx = 0; idx < 3; ++idx) {
		fscan_ret = fscanf(fin, " %d %d ", &tst->data.options.PSIG_ROWS[idx],
				&tst->data.options.PSIG_COLS[idx]);
	}
...
//    tst->p_width = 10;
//    tst->p_height = 10;
	tst->normalizedImageFrame = (uint8_t*) malloc(
			tst->data.options.bw[0] * tst->data.options.bw[1]
					* sizeof(uint8_t));
	tst->bb = (double*) malloc(4 * sizeof(double));
	tst->sf = (float*) malloc(2 * sizeof(float));
	tst->croppedImage = cvCreateImage(cvSize(tst->p_width, tst->p_height),
			IPL_DEPTH_8U, 1);
	tst->resizedImage = cvCreateImage(cvSize(tst->p_width, tst->p_height),
			IPL_DEPTH_8U, 1);
...
int flandmark_detect_base(uint8_t* face_image, FLANDMARK_Model* model,
	...
	for (int i = 0; i < q1_length; ++i) {
		...
		maximize_gdotprod(
				...
				model->data.options.PsiGS1[INDEX(i, 0, model->data.options.PSIG_ROWS[1])].disp,
				model->data.options.PsiGS1[INDEX(i, 0, model->data.options.PSIG_ROWS[1])].COLS,
				tsize);
	...
	for (int i = 0; i < q2_length; ++i) {
		...
		maximize_gdotprod(
				...
				model->data.options.PsiGS2[INDEX(i, 0, model->data.options.PSIG_ROWS[2])].disp,
				model->data.options.PsiGS2[INDEX(i, 0, model->data.options.PSIG_ROWS[2])].COLS,
				tsize);
	...
	for (int i = 0; i < q0_length; ++i) {
		// q10
		maxq10 = -FLT_MAX;
		maximize_gdotprod(&maxq10, &s0[INDEX(1, i, M)], s1_maxs, g[0],
				model->data.options.PsiGS0[INDEX(i, 0, model->data.options.PSIG_ROWS[0])].disp,
				model->data.options.PsiGS0[INDEX(i, 0, model->data.options.PSIG_ROWS[0])].COLS,
				tsize);
		s0[INDEX(5, i, M)] = s1[INDEX(1, (int)s0[INDEX(1, i, M)], 2)];
		// q20
		maxq20 = -FLT_MAX;
		maximize_gdotprod(&maxq20, &s0[INDEX(2, i, M)], s2_maxs, g[1],
				model->data.options.PsiGS0[INDEX(i, 1, model->data.options.PSIG_ROWS[0])].disp,
				model->data.options.PsiGS0[INDEX(i, 1, model->data.options.PSIG_ROWS[0])].COLS,
				tsize);
		s0[INDEX(6, i, M)] = s2[INDEX(1, (int)s0[INDEX(2, i, M)], 2)];
		// q30
		maxq30 = -FLT_MAX;
		maximize_gdotprod(&maxq30, &s0[INDEX(3, i, M)], q[3], g[2],
				model->data.options.PsiGS0[INDEX(i, 2, model->data.options.PSIG_ROWS[0])].disp,
				model->data.options.PsiGS0[INDEX(i, 2, model->data.options.PSIG_ROWS[0])].COLS,
				tsize);
		// q40
		maxq40 = -FLT_MAX;
		maximize_gdotprod(&maxq40, &s0[INDEX(4, i, M)], q[4], g[3],
				model->data.options.PsiGS0[INDEX(i, 3, model->data.options.PSIG_ROWS[0])].disp,
				model->data.options.PsiGS0[INDEX(i, 3, model->data.options.PSIG_ROWS[0])].COLS,
				tsize);
		// q70
		maxq70 = -FLT_MAX;
		maximize_gdotprod(&maxq70, &s0[INDEX(7, i, M)], q[7], g[6],
				model->data.options.PsiGS0[INDEX(i, 4, model->data.options.PSIG_ROWS[0])].disp,
				model->data.options.PsiGS0[INDEX(i, 4, model->data.options.PSIG_ROWS[0])].COLS,
				tsize);
	...
	for (int i = 0; i < M; ++i) {
		...
		landmarks[INDEX(0, i, 2)] = float(
				COL(indices[i], rows) + optionsS[INDEX(0, i, 4)]);
		landmarks[INDEX(1, i, 2)] = float(
...
int flandmark_detect(IplImage *img, int *bbox, FLANDMARK_Model *model,
	...
	getNormalizedImageFrame(img, bbox, model->bb, model->normalizedImageFrame,
			model);
...
int imCrop(IplImage *input, IplImage *output, const CvRect region,
	...
	if (model->p_width < region.width || model->p_height < region.height) {
		...
		output = cvCreateImage(cvSize(model->p_width, model->p_height),
				IPL_DEPTH_8U, 1);
	} else {
...
int imResize(IplImage *input, IplImage *output, int width, int height,
	...
	if (width <= 0 || height <= 0 || input == 0 || origWidth <= 0
			|| origHeight <= 0) {
		return 1;
	}
...
int getNormalizedImageFrame(IplImage *input, const int bbox[], double *bb,
	...
	if (imCrop(input, model->croppedImage,
			cvRect(bb[0], bb[1], bb[2] - bb[0] + 1, bb[3] - bb[1] + 1),
			model)) {
		// something was bad
		return 1;
	}
	// imresize
	if (imResize(model->croppedImage, model->resizedImage,
			model->data.options.bw[0], model->data.options.bw[1], model)) {
		// something was bad
		return 1;
	}
	...
	for (int x = 0; x < model->data.options.bw[0]; ++x) {
		for (int y = 0; y < model->data.options.bw[1]; ++y) {
			face_img[INDEX(x, y, model->data.options.bw[1])] =
					(uint8_t) ((model->resizedImage->imageData
							+ model->resizedImage->widthStep * x)[y]);
		}
	}
