...
{
	mpDetector=cv::Ptr<cv::FeatureDetector>(new cv::OrbFeatureDetector(200));
...
void PlanarObjectTracker::loadTemplate(const std::string& filepath){
	...
	//cv::Ptr<cv::FeatureDetector> pDetector(new cv::DynamicAdaptedFeatureDetector(new cv::FastAdjuster(100,true,1,200),400,500,5));
	//cv::Ptr<cv::DescriptorExtractor> pDescriptor(new cv::BriefDescriptorExtractor(32));
	...
	std::cout<<mTemplateDescriptor.type()<<std::endl;
	std::cout<<CV_8UC1<<std::endl;
	mpTempalateDescriptorIndex=cv::Ptr<cv::flann::Index>(new cv::flann::Index(mTemplateDescriptor,cv::flann::KDTreeIndexParams(1)));
