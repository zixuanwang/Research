...
function n_matched = compare_group_recommendation_allitems(feature_type,method,measure,measure_size)
    ...
    for i=1:280 
        ...
        elseif strcmp(measure, 'top_half')
                pos_matched = compute_topk_precision(pred_pos_votes,pos_votes, round(length(pos_votes)/2));  
        elseif strcmp(measure, 'top_k')
            pos_matched = compute_topk_precision(pred_pos_votes,pos_votes, measure_size);  
        else
